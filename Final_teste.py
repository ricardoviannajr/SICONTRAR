...
def criar_arquivo():
    # Pergunta ao usuário onde salvar o novo arquivo
    caminho_arquivo = tkinter.filedialog.asksaveasfilename(defaultextension='.csv')
    # Salva o caminho do arquivo em um arquivo de texto
    with open('caminho_arquivo.txt', 'w', encoding='utf-8') as arquivo:
        arquivo.write(caminho_arquivo)
    # Cria um novo arquivo CSV com o caminho especificado pelo usuário
...
def main():
    ...
    if not os.path.exists('caminho_arquivo.txt'):
        # Pergunta ao usuário se ele deseja indicar a localização ou criar um novo arquivo
        resposta = tk.messagebox.askyesnocancel('Arquivo não encontrado', 'O arquivo "dados.csv" não foi encontrado. Deseja indicar sua localização, escolha "Sim", caso queira criar um novo arquivo, escolha "Não".')

        if resposta:
            # Se o usuário escolher "Sim", pergunta ao usuário de onde importar o arquivo
            caminho_arquivo = tkinter.filedialog.askopenfilename()
            # Salva o caminho do arquivo em um arquivo de texto
            with open('caminho_arquivo.txt', 'w', encoding='utf-8') as arquivo:
                arquivo.write(caminho_arquivo)
        elif resposta is None:
            # Se o usuário escolher "Cancelar", sai da aplicação
            return
        else:
            # Se o usuário escolher "Não", cria um novo arquivo
            criar_arquivo()
    else:
        ...
        # A partir daqui, o caminho do arquivo CSV estará armazenado na variável "caminho_arquivo" e pode ser usado pelo resto da aplicação

def adicionar():
    # Abre o arquivo "titles.txt" em modo de leitura
    with open('titles.txt', 'r', encoding='utf-8') as arquivo:
        titles = arquivo.read()

    # Lê o caminho do arquivo e os títulos das colunas do arquivo de texto
    with open('caminho_arquivo.txt', 'r', encoding='utf-8') as arquivo:
        caminho_arquivo = arquivo.read()
    with open('titles.txt', 'r', encoding='utf-8') as arquivo:
        titles = arquivo.read().split(',')
    ...
    with open(caminho_arquivo, 'a', encoding='utf-8', newline='') as arquivo_csv:
        writer = csv.writer(arquivo_csv)
        # Escreve os títulos das colunas no arquivo CSV caso ele esteja vazio
...
def pesquisar():
    ...
    with open(caminho_arquivo, 'r', encoding = 'utf-8') as arquivo_csv:
        ...
        for linha in reader:
            if len(linha) >= 16:
                # Concatenamos todos os campos em uma única string
                ...
                # Verificamos se o termo de pesquisa está presente na string "dados"
